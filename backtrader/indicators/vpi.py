...
class VolumnLong(Indicator):
    '''
    Records volumn which have been "long": the close price has been
    higher than open price.

    Formula:
      - volumnlong = volumn if close > open else o
    ...
    lines = ('volumn',)
    def __init__(self):
        self.lines.volumn = self.data0 if self.data2 >= self.data1 else 0
        super(VolumnLong, self).__init__()
...
class VolumnLong(Indicator):
    ...
    lines = ('volumn',)
    def __init__(self):
        self.lines.volumn = self.data0 if self.data1 >= self.data2 else 0
        super(VolumnLong, self).__init__()
